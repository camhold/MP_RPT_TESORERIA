...
class AccountGeneralLedgerReport(models.AbstractModel):
    ...
    def _get_templates(self):
        ...
        templates['line_template'] = 'custom_account_reports.line_template_general_ledger_report'
    ...
    def _get_query_sums(self, options_list, expanded_account=None):
        ...
        # Compute only the unaffected earnings for the oldest period and add the total period if required.
        options_periods = [(len(options_list) - 1, options_list[-1])]
        if len(options_list) > 1:
            options_periods.append((0, options_list[0]))

        for i, options_period in options_periods:
