...
class AccountReport(models.AbstractModel):
    ...
    def _get_templates(self):
        return {
                'main_template': 'custom_account_reports.main_template',
                'main_table_header_template': 'custom_account_reports.main_table_header',
                'line_template': 'custom_account_reports.line_template',
                'footnotes_template': 'custom_account_reports.footnotes_template',
                'search_template': 'custom_account_reports.search_template',
                'line_caret_options': 'custom_account_reports.line_caret_options',
        }
    ...
    def open_general_ledger(self, options, params=None):
        ...
        action_vals = self.env['ir.actions.actions']._for_xml_id('custom_account_reports.action_account_report_general_ledger')
        action_vals['params'] = {
            'options': options,
            'ignore_session': 'read',
        }
    ...
    def get_html_footnotes(self, footnotes):
        template = self._get_templates().get('footnotes_template', 'custom_account_reports.footnotes_template')
    ...
    def open_report_export_wizard(self, options, extra_parameter):
        ...
        new_wizard = self.with_context(new_context).env['custom_account_reports.export.wizard'].create({'report_model': self._name, 'report_id': getattr(self, 'id', 0)})
        view_id = self.env.ref('custom_account_reports.view_report_export_wizard').id
        return {
            'type': 'ir.actions.act_window',
            'name': _('Export'),
            'view_mode': 'form',
            'res_model': 'custom_account_reports.export.wizard',
            'target': 'new',
    ...
    def get_pdf(self, options):
        ...
        body = self.env['ir.ui.view']._render_template(
            "custom_account_reports.print_template",
